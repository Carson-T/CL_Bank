...
from model.CLIP_2stage_Net import CLIP_2stage_Net
...
class CLIP_2stage(Base):
    ...
    def prepare_model(self, task_id, checkpoint=None):
        if self.model is None:
            self.model = CLIP_2stage_Net(self.config, self.logger)
            self.model.model_init()
        self.model.update_model(task_id)
        self.model.freeze_fe()
        self.model.change_adapter_grad(False)
    ...
    def incremental_train(self, data_manager, task_id):
        ...
        self.logger.info("stage 1 training")
        ...
        self.logger.info("stage 2 training")
        self.model.freeze_prompt()
        self.model.change_adapter_grad(True)
        self.model.show_trainable_params()
        optimizer = get_optimizer(filter(lambda p: p.requires_grad, self.model.parameters()), self.config)
        # optimizer = optim.SGD([{"params": self.model.img_adapter_list.parameters(), "lr": self.config.lr*0.01},
        #                        {"params": self.model.img_final_adapter.parameters(), "lr": self.config.lr}],
        ...
        if len(os.environ["CUDA_VISIBLE_DEVICES"].split(",")) > 1:
            self.model = nn.DataParallel(self.model)
        self.train_model(self.train_loader, self.test_loader, hard_loss, soft_loss, optimizer, scheduler,
                         task_id=task_id, epochs=self.config.epochs, stage2=True)
        if len(os.environ["CUDA_VISIBLE_DEVICES"].split(",")) > 1:
            self.model = self.model.module
    ...
    def train_model(self, train_loader, test_loader, hard_loss, soft_loss, optimizer, scheduler, task_id, epochs, stage2=False):
    ...
    def epoch_train(self, model, train_loader, hard_loss, soft_loss, optimizer, task_id, stage2):
        losses = 0.
        ce_losses = 0.
        model.train()
        for idx, (inputs, targets, _) in enumerate(train_loader):
            ...
            out = model(inputs, text_tokens=self.new_text_tokens.cuda(), train=True, stage2=stage2)
    ...
    def epoch_test(self, model, test_loader, hard_loss, soft_loss, task_id, stage2):
        losses = 0.
        ce_losses = 0.
        model.eval()
        with torch.no_grad():
            for idx, (inputs, targets, _) in enumerate(test_loader):
                inputs, targets = inputs.cuda(), targets.cuda()
                out = model(inputs, text_tokens=self.cur_text_tokens.cuda(), stage2=stage2)
                logits_global = out["logits"]
                if stage2:
                    out1 = model(inputs, text_tokens=self.cur_text_tokens.cuda(), stage2=False)
                    task_list = torch.div(torch.max(out1["logits"], dim=1)[1], self.config.increment_steps[0], rounding_mode="floor")
                    indices = (torch.arange(self.config.increment_steps[0]).reshape(1, -1)).cuda() + task_list.unsqueeze(-1)*self.config.increment_steps[0]
                    logits = torch.gather(logits_global, dim=-1, index=indices)
                    preds = torch.max(logits, dim=1)[1]+task_list*self.config.increment_steps[0]
                else:
    ...
    def predict(self, model, test_loader, task_id):
        ...
        with torch.no_grad():
            for idx, (inputs, targets, _) in enumerate(test_loader):
                inputs, targets = inputs.cuda(), targets.cuda()
                # out = model(inputs, text_tokens=self.cur_text_tokens.cuda(), train=False, task_id=task_id, img_proto=self.class_means.cuda())

                out1 = model(inputs, text_tokens=self.cur_text_tokens.cuda(), stage2=False)
                task_list = torch.div(torch.max(out1["logits"], dim=1)[1], self.config.increment_steps[0], rounding_mode="floor")
                out = model(inputs, text_tokens=self.cur_text_tokens.cuda(), img_proto=self.class_means.cuda(), stage2=True)
                logits_global = out["logits"]
                indices = (torch.arange(self.config.increment_steps[0]).reshape(1, -1)).cuda() + task_list.unsqueeze(-1)*self.config.increment_steps[0]
                logits = torch.gather(logits_global, dim=-1, index=indices)
                scores, preds = torch.max(F.softmax(logits, dim=-1), dim=1)
                preds = preds+task_list*self.config.increment_steps[0]

...
from model.CLIP_MoE_Net import CLIP_MoE_Net
...
class CLIP_MoE(Base):
    ...
    def prepare_model(self, task_id, checkpoint=None):
        if self.model is None:
            self.model = CLIP_MoE_Net(self.config, self.logger)
    ...
    def incremental_train(self, data_manager, task_id):
        ...
        # optimizer = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.config.lr,
        #                        momentum=0.9, weight_decay=self.config.weight_decay)
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=self.config.ca_epochs)
        ...
        # self.train_model(self.train_loader, self.test_loader, hard_loss, soft_loss, optimizer, scheduler,
        #                  task_id=task_id, epochs=self.config.ca_epochs)
        if len(os.environ["CUDA_VISIBLE_DEVICES"].split(",")) > 1:
    ...
    def train_model(self, train_loader, test_loader, hard_loss, soft_loss, optimizer, scheduler, task_id, epochs):
        ...
        for epoch in range(epochs):
            if task_id > 0:
                self.stage2_training(task_id, optimizer, epoch)
            else:
            ...
            wandb.log({
                ...
                "task " + str(task_id + 1) + "/" + "train_overall_acc": 0,
                "task " + str(task_id + 1) + "/" + "test_overall_acc": test_overall_acc,
                "task " + str(task_id + 1) + "/" + "train_loss": 0,
                "task " + str(task_id + 1) + "/" + "test_loss": test_loss["all_loss"]
            })

            self.logger.info("task_id: {}, epoch: {}/{}".format(task_id + 1, epoch + 1, epochs))
            self.logger.info(
                "train_overall_acc: {:.2f}, test_overall_acc: {:.2f}".format(0, test_overall_acc))
            self.logger.info("train_losses: {}".format(0))
    ...
    def epoch_train(self, model, train_loader, hard_loss, soft_loss, optimizer, task_id):
        ...
        for idx, (inputs, targets, _) in enumerate(train_loader):
            ...
            with autocast():
                out = model(inputs, text_tokens=self.cur_text_tokens.cuda(), train=True, task_id=task_id, targets=targets)
    ...
    def stage2_training(self, task_id, optimizer, epoch):
        # self.model.freeze_adapter()
        self.model.eval()
        losses = 0.
        sampled_data = []
